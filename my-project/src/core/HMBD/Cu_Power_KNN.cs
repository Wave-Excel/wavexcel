...
namespace HMBD.CustomPower_KNN;
public class CustomPowerKNN
{
    ...
    public void ExecutePowerKNN(string criteria)
    {
        ...
        int filteredCount = 0;
        double[] minList = new double[6];
        double[] maxList = new double[6];
        for(int i = 0; i <= 5; ++i){
        ...
        powerNearestList[0].Power = turbineDataModel.AK25;
        ...
        normNewTurbine[0] = (powerNearestList[0].Power - minList[0])/(maxList[0] - minList[0]);
        normNewTurbine[1] = (powerNearestList[0].SteamPressure - minList[1])/(maxList[1] - minList[1]);
        normNewTurbine[2] = (powerNearestList[0].SteamTemperature - minList[2])/(maxList[2] - minList[2]);
        normNewTurbine[3] = (powerNearestList[0].SteamMass - minList[3])/(maxList[3] - minList[3]);
        normNewTurbine[4] = (powerNearestList[0].ExhaustPressure - minList[4])/((maxList[4] - minList[4]));
        ...
        for (int i = 2; i <= totalRows; i++)
        {
            ...
            for (int j = 0; j < 5; j++)
            {
                distances[i - 2] += Math.Pow((double)wsPowerNormDB.Cells[i, j + 3].Value - normNewTurbine[j], 2);
            }
        ...
        for (int i = 2; i <= totalRows; i++)
        {
            ...
            switch (criteria)
            {
                ...
                if (((double)wsPowerNormDB.Cells[i, 8].Value >= 1120 && (double)wsPowerNormDB.Cells[i, 8].Value <= 1130)
                   || ((double)wsPowerNormDB.Cells[i, 8].Value >= 1190 && (double)wsPowerNormDB.Cells[i, 8].Value <= 1210)){
        ...
        for (int i = 0; i < k; i++)
        {
            ...
            powerNearestList[outputRow].SteamPressure = (double)wsPowerNormDB.Cells[filteredIndexes[i], 4].Value * (maxList[1] - minList[1]) + minList[1];
            powerNearestList[outputRow].SteamTemperature = (double)wsPowerNormDB.Cells[filteredIndexes[i], 5].Value * (maxList[2] - minList[2]) + minList[2];
            powerNearestList[outputRow].SteamMass = (double)wsPowerNormDB.Cells[filteredIndexes[i], 6].Value * (maxList[3] - minList[3]) + minList[3];
            powerNearestList[outputRow].ExhaustPressure = (double)wsPowerNormDB.Cells[filteredIndexes[i], 7].Value * (maxList[4] - minList[4]) + minList[4];
            powerNearestList[outputRow].Bcd = Convert.ToInt32(wsPowerNormDB.Cells[filteredIndexes[i], 8].Value);
            outputRow++;
        }
